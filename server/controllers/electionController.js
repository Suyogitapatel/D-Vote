...
const addElection = async (req, res, next) =>{
    ...
    if(!req.files.thumbnail){
        return next(new HttpError ("Choose the thumbnail.", 422))
    }
    ...
    await thumbnail.mv(path.join(__dirname, '..', 'upload', fileName), async (err) => {
        if(err){
            return next(HttpError(err))
        }
        // store image on cloudinary
        const result = await cloudinary.uploader.upload(path.join(__dirname, '..', "upload", fileName), {resource_type: "image"})
...
const updateElection =async (req, res, next) =>{
    try {
        // only admin can add election
    if(!req.user.isAdmin){
        return next(new HttpEroor("Only an admin can perform this action.", 403))
    }
        const {id} = req.params;
        const {title, discription} = req.body;
        if(!title || !discription){
        ...
        if(req.files.thumbnail){
            ...
            // image should be less than 1mb
            if(thumbnail.size > 1000000){
                return next(new HttpError("Image size is too big, should be less than 1mb.", 422))
            }
            //rename the image
    let fileName = thumbnail.name;
    fileName = fileName.split(".")
